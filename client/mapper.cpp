...
*/
void mapper::comms::wait_for_op_to_end()
{
...
bool mapper::comms::is_address_valid(uint64_t address)
{
...
void mapper::comms::unload_payload()
{
	wait_for_op_to_end();
...
void mapper::comms::mm_copy_virtual_memory(uint32_t source_pid, uint64_t source_address, uint32_t target_pid, 
...
uint64_t mapper::comms::get_base_address(uint32_t pid)
{
	wait_for_op_to_end();

	shared_memory.memory_operation_params.target_pid = pid;
	shared_memory.operation = op_base;

	wait_for_op_to_end();

	return shared_memory.memory_operation_params.base_address;
}

#define STATUS_ERROR 1

NTSTATUS mapper::comms::init(char* cp_driver_path)
{
	const std::string driver_path = cp_driver_path;
	...
	if (!mapper::map_driver(iqvw64e_device_handle, driver_path))
	{
...
uint64_t mapper::map_driver(HANDLE iqvw64e_device_handle, const std::string& driver_path)
{
	...
	do
	{
		...
		*/
		...
		memset((void*)(&mapper::comms::shared_memory), 0, sizeof(mapper::comms::mapped_memory));
		comms::shared_memory.operation = mapper::comms::op_complete;
...
void mapper::relocate_image_by_delta(portable_executable::vec_relocs relocs, const uint64_t delta)
{
...
bool mapper::resolve_imports(HANDLE iqvw64e_device_handle, portable_executable::vec_imports imports)
{
