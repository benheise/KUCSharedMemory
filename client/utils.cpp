...
bool utils::read_file_to_memory(const std::string& file_path, std::vector<uint8_t>* out_buffer)
{
...
bool utils::create_file_from_memory(const std::string& desired_file_path, const char* address, size_t size)
{
...
uint32_t utils::get_process_id(const wchar_t* image_name)
{
	...
	if (Process32FirstW(snapshot, &process)) {
		do {
			if (!wcscmp(image_name, process.szExeFile)) {
