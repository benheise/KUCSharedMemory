...
namespace comms
{
	...
	*/
	...
	bool get_base_address() {
		PEPROCESS target_process;

		if (NT_SUCCESS(PsLookupProcessByProcessId(HANDLE(remapped_memory->memory_operation_params.target_pid), &target_process)))
		{
			remapped_memory->memory_operation_params.base_address = reinterpret_cast<uint64_t>(PsGetProcessSectionBaseAddress(target_process));

			DbgPrintEx(0, 0, "Base address: %p\n", remapped_memory->memory_operation_params.base_address);
